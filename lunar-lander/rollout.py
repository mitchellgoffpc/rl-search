...
def search_best_action(seed, action_history, num_simulations):
    ...
    for action in actions:
        for _ in range(num_simulations):
            ...
            for a in action_history:
                env.step(a)
            reward = run_simulation(env, action)
            action_results[action].append(reward)
...
def run_episode(model_path, num_simulations, epsilon, render=False, save=False):
    ...
    _, total_reward, _, _, _ = env.step(action)  # state is a numpy array on the first iter, after the first step it becomes a tuple
    ...
    while not done and not trunc:
        ...
        if save or not model:  # only compute the tree decision if we have to
            tree_decision, _ = search_best_action(seed, actions, num_simulations)
    ...
    return steps, total_reward, observations, actions, tree_decisions, et - st
...
def run_multiple_episodes(model_path, episode_dir, num_episodes, num_simulations, epsilon):
    ...
    steps, rewards, _, _, _, times = zip(*results)
...
if __name__ == "__main__":
    ...
    if args.render:
        steps, reward, *_ = run_episode(args.model, args.num_simulations, args.epsilon, render=True)
        print(f"Episode finished after {steps} steps with a total reward of {reward:.3f}")
    else:
